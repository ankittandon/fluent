...
sign_dmg_if_possible() {
    ...
    if "$SYSTEM_CODESIGN" --force --sign "$CODESIGN_IDENTITY" --timestamp "$DMG_FILE"; then
        "$SYSTEM_CODESIGN" --verify --verbose=2 "$DMG_FILE"
        return 0
    fi

    echo "Warning: DMG signing failed with identity $CODESIGN_IDENTITY; leaving DMG unsigned for local distribution." >&2
    return 0
}
