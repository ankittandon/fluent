...
const DETAILED_SEGMENT_MAX_CHARS: i32 = 48;
const DETAILED_SEGMENT_MAX_TOKENS: i32 = 24;
...
pub struct TimedTranscriptionSegment {
    pub start_ms: u64,
    pub end_ms: u64,
...
#[derive(Debug)]
pub struct DetailedTranscriptionOutput {
    pub text: String,
    pub segments: Vec<TimedTranscriptionSegment>,
    pub profile: TranscriptionProfile,
}
...
impl Transcriber {
    ...
    pub fn transcribe_detailed_profiled(
        &self,
        samples: &[f32],
    ) -> Result<DetailedTranscriptionOutput, String> {
        let total_t0 = Instant::now();
        let state_t0 = Instant::now();
        let mut state = self.create_state()?;
        let mut state_acquire = state_t0.elapsed();

        let (mut segments, mut inference, mut extract) =
            self.run_detailed_with_state(&mut state, samples, DecodePreset::Fast)?;
        let mut text = join_segment_text(&segments);

        if let Some(reason) = conservative_retry_reason(samples, &text) {
            eprintln!(
                "[screamer] Retrying detailed decode with conservative beam search ({reason})"
            );
            ...
            let (retry_segments, retry_inference, retry_extract) = self.run_detailed_with_state(
                &mut retry_state,
                samples,
                DecodePreset::Conservative,
            )?;
            let retry_text = join_segment_text(&retry_segments);
        ...
        Ok(DetailedTranscriptionOutput {
            text,
            segments,
            profile: TranscriptionProfile {
                state_acquire,
    ...
    fn run_detailed_with_state(
        &self,
        state: &mut WhisperState,
        samples: &[f32],
        preset: DecodePreset,
    ) -> Result<(Vec<TimedTranscriptionSegment>, Duration, Duration), String> {
        let mut params = FullParams::new(match preset {
            DecodePreset::Fast => SamplingStrategy::Greedy { best_of: 1 },
            DecodePreset::Conservative => SamplingStrategy::BeamSearch {
                beam_size: 2,
        ...
        params.set_max_len(DETAILED_SEGMENT_MAX_CHARS);
        params.set_max_tokens(DETAILED_SEGMENT_MAX_TOKENS);
        ...
        for i in 0..num_segments {
            ...
            let text = normalize_segment_text(&text);
            if text.is_empty() {
                continue;
            }

            let start_ms = timestamp_units_to_ms(
                state
                    .full_get_segment_t0(i)
                    .map_err(|e| format!("Failed to get segment start: {}", e))?,
            );
            let end_ms = timestamp_units_to_ms(
                state
                    .full_get_segment_t1(i)
                    .map_err(|e| format!("Failed to get segment end: {}", e))?,
            )
            .max(start_ms + WHISPER_TIMESTAMP_UNIT_MS);

            segments.push(TimedTranscriptionSegment {
                start_ms,
                end_ms,
        ...
        Ok((segments, inference, extract_t0.elapsed()))
    }
...
fn join_segment_text(segments: &[TimedTranscriptionSegment]) -> String {
    segments
        .iter()
...
fn timestamp_units_to_ms(timestamp: i64) -> u64 {
